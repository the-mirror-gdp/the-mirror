...
func _get_callable_for_event_signal() -> Callable:
	return _on_object_variable_tweened


func _on_object_variable_tweened(object: Object, variable_name: String, from_value: Variant, to_value: Variant, duration: float) -> void:
	if inputs.size() > 0:
	...
	outputs[2].value = from_value
	outputs[3].value = to_value
	outputs[4].value = duration
	_execute()
