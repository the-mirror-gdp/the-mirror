...
func _execute_callback(_stack_count: int) -> Error:
	...
	if Zone.is_host():
		if target_player is Player:
			...
			GameUI.instance.scoreboard_window.set_scoreboard_shown_network.rpc_id(player_peer_id, true, allow_close, allow_new_match)
		else:
			GameUI.instance.scoreboard_window.set_scoreboard_shown(true, allow_close, allow_new_match)
	else:
		...
		GameUI.instance.scoreboard_window.set_scoreboard_shown_network(true, allow_close, allow_new_match)
	return OK
