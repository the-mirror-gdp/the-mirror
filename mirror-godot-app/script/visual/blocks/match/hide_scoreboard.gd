...
func _execute_callback(_stack_count: int) -> Error:
	...
	if Zone.is_host():
		if target_player is Player:
			...
			GameUI.scoreboard_window.set_scoreboard_shown_network.rpc_id(player_peer_id, false, false, false)
		else:
		...
		GameUI.scoreboard_window.set_scoreboard_shown_network(false, false, false)
	return OK
