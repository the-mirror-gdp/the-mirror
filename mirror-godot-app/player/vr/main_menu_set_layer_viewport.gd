...
func _process(_delta):
	...
	if not _mouse_enabled and was_pressed:
		var event : InputEventMouseButton = InputEventMouseButton.new()
		event.button_index = 1
		event.pressed = false
		layer_viewport.push_input(event)
		print("DISABLED MOUSE PRESS SHOULD NOT FIRE")
		was_pressed = false
		return
...
func set_mouse_state(state: bool) -> void:
	print("VR Mouse enabled: ", state)
	_mouse_enabled = state
