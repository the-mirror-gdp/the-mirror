...
export async function POST(req: NextRequest) {
  try {
    ...
    const unzippedFolderPath = await unzipFile(file.filepath, destinationFilePath);
    const settingsFilePath = path.join(unzippedFolderPath, '__settings__.import.js');

    if (fs.existsSync(settingsFilePath)) {
      await modifySettings(settingsFilePath);
      return NextResponse.json({ message: 'File uploaded and modified successfully!' });
    } else {
      return NextResponse.json({ error: 'Settings file not found in the uploaded zip' }, { status: 404 });
    }
  } catch (error) {
    console.error('Error processing the uploaded file:', error);
    return NextResponse.json({ error: 'Error processing the uploaded file' }, { status: 500 });
  }
}

// Unzip the file
async function unzipFile(filePath: string, destinationFilePath: string): Promise<string> {
