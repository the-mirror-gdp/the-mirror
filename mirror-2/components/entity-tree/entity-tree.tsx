...
type TreeDataNodeWithEntityData = TreeDataNode & {
  name: string
  id: string
...
const EntityTree: React.FC = () => {
  ...
  useEffect(() => {
    if (entities && entities.length > 0) {
      ...
      if (
        currentScene &&
        !automaticallyExpandedSceneIds.includes(currentScene.id)
      ) {
        ...
        dispatch(
          insertAutomaticallyExpandedSceneIds({ sceneId: currentScene.id })
        )
      } else {
  ...
  useEffect(() => {
    dispatch(
      addExpandedEntityIds({
        entityIds: expandedKeys.map((key) => String(key))
      })
    )
  }, [expandedKeys])
    ...
    if (!info.dropToGap) {
      ...
      loop(data, dropKey, (item) => {
        ...
        item.children.forEach((child, index) => {
          ...
          if (
            !entitiesUpdateArray.some((entity) => entity.id === updateData.id)
          ) {
  ...
  const onExpand: TreeProps['onExpand'] = (expandedKeysValue) => {
    ...
    dispatch(
      setExpandedEntityIds({
        entityIds: expandedKeysValue.map((key) => String(key))
      })
    )
  }
