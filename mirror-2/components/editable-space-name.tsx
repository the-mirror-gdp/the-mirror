...
export function EditableSpaceName() {
  ...
  const { data: space, isLoading, isSuccess, error } = useGetSingleSpaceQuery(params.spaceId)
  ...
  return (
    isLoading ? <Skeleton className="w-full dark:bg-transparent border-none text-lg shadow-none md:w-2/3 lg:w-1/3"></Skeleton>
      :
      <Form {...form} >
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full" onBlur={form.handleSubmit(onSubmit)}>
          <FormField
